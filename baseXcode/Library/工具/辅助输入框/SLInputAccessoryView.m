...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 35);
        self.backgroundColor = [UIColor whiteColor];
        UIButton * btn = [UIButton buttonWithTitle:@"完成" titleColor:[UIColor darkGrayColor] backgroundColor:nil font:14 image:nil frame:Rect([UIScreen mainScreen].bounds.size.width - 60, 0, 40, self.frame.size.height)];
