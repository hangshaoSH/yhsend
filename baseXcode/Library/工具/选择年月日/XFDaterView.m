...
- (void)initViews{
    ...
    daterContentView = [[UIView alloc]initWithFrame:Rect(0, kScreenHeigth-220, kScreenWidth, 220)];
...
//    daterContentView.layer.cornerRadius=8;
    ...
    daterLab=[[UILabel alloc]initWithFrame:Rect(14, 0, daterContentView.width, 30)];
    daterLab.text=@"请选择时间";
    daterLab.backgroundColor=WhiteColor;
    daterLab.font=[UIFont systemFontOfSize:15];
    daterLab.textColor=BlackFontColor;
    daterLab.textAlignment=0;
    [daterContentView addSubview:daterLab];
    
    picker = [[UIPickerView alloc]initWithFrame:Rect(20, daterLab.height, kScreenWidth, 130)];
    picker.dataSource=self;
    picker.delegate=self;
    [daterContentView addSubview:picker];
    
    cancelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    cancelBtn.frame = Rect(0, 170, daterContentView.width/2, 50);
    [cancelBtn setTitle:@"确定" forState:UIControlStateNormal];
    [cancelBtn addTarget:self action:@selector(cancelAction:) forControlEvents:UIControlEventTouchUpInside];
    [daterContentView addSubview:cancelBtn];
    cancelBtn.backgroundColor=[UIColor colorWithHexString:@"ffb400"];
    
    sureBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    sureBtn.frame = Frame(daterContentView.w/2.0, 170, daterContentView.w/2.0, cancelBtn.h);
    [sureBtn setTitle:@"取消" forState:UIControlStateNormal];
    [sureBtn addTarget:self action:@selector(sureAction:) forControlEvents:UIControlEventTouchUpInside];
    [daterContentView addSubview:sureBtn];
    sureBtn.backgroundColor=[UIColor colorWithHexString:@"b5b5b5"];
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    if (_dateViewType == XFDateViewTypeDate) {
        if(component == 0){
            rowLab.text = [NSString stringWithFormat:@"%d年",[yearArr[row%yearArr.count] intValue]-150];
        }else if(component == 1){
