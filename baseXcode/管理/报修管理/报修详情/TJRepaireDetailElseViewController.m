//
//  TJRepaireDetailElseViewController.m
//  baseXcode
//
...
@interface TJRepaireDetailElseViewController ()<UITableViewDelegate,UITableViewDataSource,TJRepaireDetailNextTopDelegate,CuiPickViewDelegate,UITextFieldDelegate,SDPhotoBrowserDelegate,TJRepaireTopDelegate,TJRepaireStepBaseDelegate,TJRepaireStepBottomDelegate>
...
@property (nonatomic,   assign) NSInteger  lookImageFlag;
@property (nonatomic,   assign) NSInteger  myrow;
@property (nonatomic,   assign) CGFloat   orginNumber;
@property (nonatomic,   strong) NSMutableArray     * imageArray;
@property (nonatomic,   assign) NSInteger   returnRefresh;
@end

@implementation TJRepaireDetailElseViewController
...
- (void)setTableView
{
    ...
    wself.ts_navgationBar = [TSNavigationBar navWithTitle:@"报修单详情" backAction:^{
        ...
        if (self.returnRefresh == 1 && self.jumpFlag == 1) {
            if (self.MyBlock) {
                self.MyBlock();
            }
        }
...
{
    if ([self.dataDic count] == 0) {//5已完成未回访  6已完成已回访  9不维修
        return 1;
    }
    if (section == 0) {
        return 3;
...
-(CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        return height + 95 + 3 * fifteenS.height;
    }else {
        if (indexPath.row == 0) {
            NSInteger height = 0;
            if ([self.dataDic[@"procinfo"] isKindOfClass:[NSString class]]) {
                ...
                if ([self.dataDic[@"backreply"] length] == 0) {
                    return 96+2*fifteenS.height + height;
                }else {
                    CGSize s = [self.dataDic[@"backreply"] getStringRectWithfontSize:15*ScaleModel width:kScreenWidth-42];
                    return 96+fifteenS.height+s.height + height;
                }
        } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            if ([self.dataDic[@"procinfo"] count] == 0) {
                static NSString * ID = @"TJRepaireStepBottomOnlyCell";
                UITableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:ID];
                if (cell == nil) {
                    cell = [[[NSBundle mainBundle] loadNibNamed:@"TJRepaireElseDetailCell" owner:self options:nil] objectAtIndex:6];
                }
                ...
                UILabel * content = (UILabel *)[cell.contentView viewWithTag:100];
                content.font = [UIFont systemFontOfSize:15 * ScaleModel];
                if ([self.dataDic[@"backreply"] length]== 0) {
                    content.text = @"无说明";
                }else {
                    content.text = self.dataDic[@"backreply"];
                }
                UIView * bgview = (UIView *)[cell.contentView viewWithTag:200];
            ...
            TJRepaireStepBottomTableViewCell * cell = [TJRepaireStepBottomTableViewCell cellWithTableView:tableView];
            [cell setCellWithDic:self.dataDic];
            [cell setDelegate:self];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            return cell;
            ...
            if ([self.dataDic[@"isvisit"] integerValue] == 1) {
                ...
                [sure addTarget:self action:@selector(notRepaireDetailSuerAc)];
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
                return cell;
            }
        }
...
//            return;
    ...
    if (button.tag == 112) {
        ...
        [[HSMidlabelView sharedInstance] setOriginX:originxS.width + 37 andOriginY: 64 + self.tableView.contentSize.height - self.scrollY - 161 - fifteenS.height * 2 - 34 * array.count + 6 andWidth:133 andHeight:34 * array.count andData:array withString:self.peopleAppraise.text clickAtIndex:^(NSMutableDictionary * dic) {
...
#pragma mark - delete
- (void)imageBaseStepSelect:(UICollectionView *)collectionView index:(NSInteger)index row:(NSInteger)row
{
    self.myrow = row;
    self.lookImageFlag = 1;
    SDPhotoBrowser *browser = [[SDPhotoBrowser alloc] init];
    browser.sourceImagesContainerView = collectionView; // 原图的父控件
    NSArray * array = [NSArray array];
    array = [self.dataDic[@"procinfo"][row][@"procpic"] componentsSeparatedByString:@"^"];
    browser.imageCount = [array count];
...
- (void)notRepaireDetailSuerAc//报修单回访
{
