//
//  TJReserveSearchViewController.m
//  baseXcode
//
...
@interface TJReserveSearchViewController ()<UITableViewDelegate,UITableViewDataSource,XFDaterViewDelegate>
...
@implementation TJReserveSearchViewController
...
{
    return 208 - 4 * 18 + 4*fifteenS.height;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"TJReserveManagerCell" owner:self options:nil] objectAtIndex:7];
    }
    for (int i = 0; i < 5; i ++) {
...
- (void)allRepairbutAc:(UIButton *)button
{
    ...
    if (button.tag == 112) {//类型
        ...
        [[HSMidlabelView sharedInstance] setOriginX:s.width + 37 andOriginY: 64 + 144+(fifteenS.height - 18)*3/2 andWidth:133 andHeight:(kScreenHeigth - (64 + 144+(fifteenS.height - 18)*3/2)) andData:array withString:self.style.text clickAtIndex:^(NSMutableDictionary * dic) {
            ...
            self.dataDic[@"storetype"] = @([dic[@"index"] integerValue] - 1);
        }];
    }
